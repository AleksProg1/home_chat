...
class ChatPresenter:
    ...
    def present(self, message: ServerMessage) -> list[ViewEvent]:
        if message.event == ServerEvent.WELCOME:
            return self._info_with_users(
                f"Connected as {message.username}",
                message.users,
            )

        if message.event == ServerEvent.USER_JOINED:
            return self._info_with_users(
                f"{message.username} joined",
                message.users,
            )

        if message.event == ServerEvent.USER_LEFT:
            return self._info_with_users(
                f"{message.username} left",
                message.users,
            )
        ...
        return [
            ViewEvent(
                kind=ViewEventKind.INFO,
                text=str(message.event),
            )
        ]

    def disconnected(self, reason: str = "Disconnected") -> ViewEvent:
        return ViewEvent(
            kind=ViewEventKind.DISCONNECTED,
            text=reason,
        )

    def _info_with_users(
        self,
        text: str,
        users: list[str] | None,
    ) -> list[ViewEvent]:
        return [
            ViewEvent(kind=ViewEventKind.INFO, text=text),
            self._users_event(users),
        ]
    ...
    def _message_event(self, message: ServerMessage) -> ViewEvent:
        ...
        is_private = message.scope == MessageScope.UNICAST
